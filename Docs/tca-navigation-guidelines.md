...
현재 PopPang에서는 홈에서 시작하는 search / popup request fullScreen 흐름도 이 owner destination에 두고, `HomeFeature`는 delegate intent만 올리는 방향을 사용한다. popup request UI 구현체가 RN이어도 이 owner는 바꾸지 않는다.
